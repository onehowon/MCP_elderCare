...
const cm: Record<Label, Record<Label, number>> = {
...
(async () => {
  ...
  const macro = {
    precision: +(metrics.reduce((s, m) => s + m.precision, 0) / metrics.length).toFixed(3),
    recall: +(metrics.reduce((s, m) => s + m.recall, 0) / metrics.length).toFixed(3),
    f1: +(metrics.reduce((s, m) => s + m.f1, 0) / metrics.length).toFixed(3),
  };

  console.log("Macro Avg:", macro);

  const total_high = Object.values(cm["high"]).reduce((a, b) => a + b, 0);
  const miss_high = cm["high"]["low"] + cm["high"]["mid"];
  console.log(`HR-Miss: ${(miss_high / total_high).toFixed(3)}`);
})();
